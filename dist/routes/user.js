...
router.get('/profile', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT u.id, u.email, u.name AS user_name, u.emoji, u.couple_id, u.created_at,
   ...
   WHERE u.id = $1`, [userId]);
        ...
        if (hasPartner) {
            ...
            const partnerResult = await (0, db_1.query)(`SELECT u.id, u.email, u.name, u.emoji,
...
router.put('/profile', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const profileFieldsProvided = ['dateOfBirth', 'major', 'year', 'hobby'].some((field) => (req.body || {})[field] !== undefined);
        ...
        await (0, db_1.withTransaction)(async (client) => {
            ...
            if (profileFieldsProvided) {
                await client.query(`INSERT INTO profiles (user_id, date_of_birth, major, year, hobby, created_at, updated_at)
        ...
        const updatedProfile = await (0, db_1.query)(`SELECT u.id, u.email, u.name AS user_name, u.emoji, u.couple_id, u.created_at,
              p.date_of_birth, p.major, p.year, p.hobby,
              c.user1_id, c.user2_id
       FROM users u
       LEFT JOIN profiles p ON p.user_id = u.id
 ...
 */
router.post('/partner/generate-code', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const userCheck = await (0, db_1.query)('SELECT couple_id FROM users WHERE id = $1', [userId]);
        ...
        await (0, db_1.query)('DELETE FROM pairing_codes WHERE user_id = $1 AND used = FALSE', [userId]);
        ...
        await (0, db_1.query)('INSERT INTO pairing_codes (code, user_id, expires_at, created_at) VALUES ($1, $2, $3, NOW())', [code, userId, expiresAt]);
 ...
 */
router.post('/partner/connect', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const result = await (0, db_1.withTransaction)(async (client) => {
            ...
            const codeResult = await client.query('SELECT user_id, expires_at, used FROM pairing_codes WHERE code = $1', [code]);
            ...
            const codeOwnerCheck = await client.query('SELECT couple_id FROM users WHERE id = $1', [codeData.user_id]);
            if (codeOwnerCheck.rows[0]?.couple_id) {
                throw new Error('CODE_OWNER_HAS_PARTNER');
            }
            // Create couple
            const coupleResult = await client.query('INSERT INTO couples (user1_id, user2_id, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING id', [codeData.user_id, userId]);
            const coupleId = coupleResult.rows[0].id;
            // Update both users with couple_id
            await client.query('UPDATE users SET couple_id = $1, updated_at = NOW() WHERE id IN ($2, $3)', [coupleId, codeData.user_id, userId]);
            // Mark code as used
            await client.query('UPDATE pairing_codes SET used = TRUE WHERE code = $1', [code]);
            return { coupleId, partnerId: codeData.user_id };
        });
        // Get partner details
        const partnerResult = await (0, db_1.query)('SELECT id, email, name FROM users WHERE id = $1', [result.partnerId]);
        res.json({
            success: true,
            data: {
 ...
 */
router.get('/partner', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const result = await (0, db_1.query)(`SELECT u.couple_id, c.user1_id, c.user2_id, c.created_at,
 ...
 */
router.delete('/partner/unmatch', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        await (0, db_1.withTransaction)(async (client) => {
            ...
            await client.query('UPDATE users SET couple_id = NULL, updated_at = NOW() WHERE couple_id = $1', [coupleId]);
