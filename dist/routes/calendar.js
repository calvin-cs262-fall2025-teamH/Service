 ...
 */
async function getUserPartnershipId(userId) {
...
router.post('/events', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`INSERT INTO calendar_events
       ...
       RETURNING *`, [partnershipId, userId, title, description || null, date, time || null, endTime || null, isAllDay !== false, eventType || 'other', location || null]);
...
router.get('/events', auth_1.authenticateToken, async (req, res) => {
    try {
       ...
       ORDER BY ce.event_date ASC, ce.event_time ASC NULLS LAST`, [partnershipId]);
        res.json({
            success: true,
 ...
 */
router.get('/events/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
       ...
       FROM calendar_events ce
       LEFT JOIN users u ON ce.added_by_user_id = u.id
       WHERE ce.id = $1 AND ce.partnership_id = $2`, [eventId, partnershipId]);
 ...
 */
router.put('/events/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const eventId = parseInt(req.params.id);
        const { date, time, endTime, title, description, eventType, isAllDay, location } = req.body;
        ...
        const result = await (0, db_1.query)(`UPDATE calendar_events
       ...
       RETURNING *`, [date, time, endTime, title, description, eventType, isAllDay, location, eventId, partnershipId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
 ...
 */
router.delete('/events/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const eventId = parseInt(req.params.id);
        const partnershipId = await getUserPartnershipId(userId);
        ...
        const result = await (0, db_1.query)(`DELETE FROM calendar_events
       WHERE id = $1
         AND partnership_id = $2
       RETURNING id`, [eventId, partnershipId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Calendar event not found or access denied'
 ...
 */
router.get('/anniversaries', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const partnershipId = await getUserPartnershipId(userId);
        if (!partnershipId) {
        ...
        const coupleResult = await (0, db_1.query)('SELECT created_at FROM couples WHERE id = $1', [partnershipId]);
