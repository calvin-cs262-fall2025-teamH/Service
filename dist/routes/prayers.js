...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Create prayer item
        const result = await (0, db_1.query)(`INSERT INTO prayer_items (couple_id, added_by_user_id, title, description, is_answered, created_at, updated_at)
       VALUES ($1, $2, $3, $4, FALSE, NOW(), NOW())
       RETURNING id, couple_id, added_by_user_id, title, description, is_answered, answered_at, created_at, updated_at`, [coupleId, userId, title.trim(), content.trim()]);
...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT id, couple_id, title, description, is_answered, answered_at, created_at, updated_at
       FROM prayer_items
       WHERE couple_id = $1
       ORDER BY is_answered ASC, created_at DESC, id DESC`, [coupleId]);
        res.json({
            success: true,
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT id, couple_id, title, description, is_answered, answered_at, created_at, updated_at
       FROM prayer_items
       WHERE id = $1 AND couple_id = $2`, [prayerId, coupleId]);
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
       ...
       RETURNING id, couple_id, title, description, is_answered, answered_at, created_at, updated_at`, [title, content, prayerId, coupleId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Prayer item not found'
 ...
 */
router.put('/:id/toggle-answered', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const prayerId = parseInt(req.params.id);
        if (isNaN(prayerId)) {
        ...
        // Get current state
        const currentResult = await (0, db_1.query)('SELECT is_answered FROM prayer_items WHERE id = $1 AND couple_id = $2', [prayerId, coupleId]);
       ...
       RETURNING id, couple_id, title, description, is_answered, answered_at, created_at, updated_at`, [newAnswered, prayerId, coupleId]);
        const prayer = result.rows[0];
        res.json({
            success: true,
            data: {
...
router.delete('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)('DELETE FROM prayer_items WHERE id = $1 AND couple_id = $2 RETURNING id', [prayerId, coupleId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Prayer item not found'
