 ...
 */
async function getUserCoupleId(userId) {
    const result = await (0, db_1.query)('SELECT couple_id FROM users WHERE id = $1', [userId]);
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Create activity
        const result = await (0, db_1.query)(`INSERT INTO activities (couple_id, title, description, date, location, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
       RETURNING id, couple_id, title, description, date, location, created_at`, [coupleId, title, description || null, activityDate, location || null]);
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const activityResult = await (0, db_1.query)(`SELECT id, couple_id, title, description, date, location, created_at
       FROM activities
       WHERE id = $1 AND couple_id = $2`, [activityId, coupleId]);
        ...
        const photosResult = await (0, db_1.query)(`SELECT id, photo_url, caption, created_at
       FROM photo_collages
       WHERE activity_id = $1
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`UPDATE activities
       ...
       RETURNING id, couple_id, title, description, date, location, created_at`, [title, description, date, location, activityId, coupleId]);
...
router.delete('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)('DELETE FROM activities WHERE id = $1 AND couple_id = $2 RETURNING id', [activityId, coupleId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Activity not found'
 ...
 */
router.post('/:id/photos', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const activityId = parseInt(req.params.id);
        ...
        const activityCheck = await (0, db_1.query)('SELECT id FROM activities WHERE id = $1 AND couple_id = $2', [activityId, coupleId]);
        ...
        // Add photo
        const result = await (0, db_1.query)(`INSERT INTO photo_collages (activity_id, photo_url, caption, created_at)
       VALUES ($1, $2, $3, NOW())
       RETURNING id, activity_id, photo_url, caption, created_at`, [activityId, photoUrl.trim(), caption || null]);
 ...
 */
router.delete('/:activityId/photos/:photoId', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const result = await (0, db_1.query)(`DELETE FROM photo_collages pc
       USING activities a
       WHERE pc.activity_id = a.id
         AND pc.id = $1
         AND pc.activity_id = $2
         AND a.couple_id = $3
       RETURNING pc.id`, [photoId, activityId, coupleId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
 ...
 */
router.get('/timeline/all', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
       ...
       WHERE a.couple_id = $1
       GROUP BY a.id
       ORDER BY a.date DESC, a.created_at DESC`, [coupleId]);
        res.json({
            success: true,
            data: result.rows.map(row => ({
                id: row.id,
