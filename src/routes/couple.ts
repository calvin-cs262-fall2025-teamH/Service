  ...
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const coupleResult = await client.query(
        `INSERT INTO couples (invite_code, user1_id, created_at, updated_at)
         VALUES ($1, $2, NOW(), NOW())
         RETURNING id, invite_code, user1_id, created_at`,
        [inviteCode, userId]
      );
      ...
      await client.query(
        'UPDATE users SET couple_id = $1, updated_at = NOW() WHERE id = $2',
        [couple.id, userId]
      );
...
router.post('/join', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    // Use transaction for atomic operation
    const coupleId = await withTransaction(async (client) => {
      // Check if user already has a couple
      const userCheck = await client.query(
        'SELECT couple_id FROM users WHERE id = $1',
        [userId]
...
router.delete('/leave', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    await withTransaction(async (client) => {
      ...
      await client.query(
        'UPDATE users SET couple_id = NULL, updated_at = NOW() WHERE id = $1',
        [userId]
      );
      ...
      if (couple.user1_id === userId && couple.user2_id) {
        await client.query(
          'UPDATE couples SET user1_id = $1, user2_id = NULL, updated_at = NOW() WHERE id = $2',
          [couple.user2_id, couple.id]
        );
      }
      // If user is user2, just clear user2_id
      else if (couple.user2_id === userId) {
        await client.query(
          'UPDATE couples SET user2_id = NULL, updated_at = NOW() WHERE id = $1',
          [couple.id]
        );
      }
